...
const getDetailedMonthlyPlan = (nextRole: string): { month: number; learning: string; practicing: string; implementing: string }[] => {
  ...
  if (role.includes("talent acquisition executive")) {
    return [
      ...
      { month: 6, learning: "Manage a mini project schedule 60+ interviews", practicing: "Track candidate drop-off points", implementing: "Analyze, suggest solutions" },
      ...
      { month: 8, learning: "Shadow TA Manager on analytics review", practicing: "Build TA activity report with metrics", implementing: "Compare with team avg, present gaps" },
  ...
  // HR Generalist / HR Operations Specialist
  if (role.includes("hr generalist") || role.includes("hr operations")) {
    return [
      { month: 1, learning: "Deep dive HR compliance & laws", practicing: "Do mini-audit of existing HR policies", implementing: "Write audit report to supervisor" },
  ...
  if (role.includes("recruiter") || role.includes("recruitment manager")) {
    return [
      ...
      { month: 3, learning: "Shadow hiring manager on JD creation", practicing: "Lead first round screening", implementing: "Compare outcomes to role profile" },
      ...
      { month: 7, learning: "Take compliance course on equal recruiting", practicing: "Review all candidate records for audit", implementing: "Address/document any compliance gaps" },
      ...
      { month: 9, learning: "Share sourcing success in HR team meet", practicing: "Pilot social recruiting campaign", implementing: "Measure applications generated" },
  ...
  // Learning & Development Specialist
  if (role.includes("learning") && role.includes("development")) {
    return [
      ...
      { month: 4, learning: "Attend digital learning workshop", practicing: "Deploy a microlearning pilot", implementing: "Measure and present engagement data" },
  ...
  // Employee / Labor Relations Specialist (2-7 years)
  if (role.includes("employee relations") || role.includes("labor relations")) {
    return [
      ...
      { month: 10, learning: "Assist in team training on current labor relations", practicing: "Lead 1 small labor relations update newsletter", implementing: "Gather training effectiveness data" },
  ...
  if (role.includes("hr manager") || role.includes("hr director")) {
    return [
      ...
      { month: 5, learning: "Attend diversity & inclusion certification", practicing: "Measure company-wide DEI initiatives", implementing: "Measure participation & impact" },
      ...
      { month: 12, learning: "Complete unit annual performance", practicing: "Set personal development goals", implementing: "Plan roadmap to Director to CHRO transition" }
    ];
  }
  
  // HR Business Partner (HRBP) (7-10 years)
  if (role.includes("hr business partner") || role.includes("hrbp")) {
    return [
      ...
      { month: 7, learning: "Attend inspiration and influencing workshops", practicing: "Coach managers on people management", implementing: "Log progress and feedback" },
      ...
      { month: 9, learning: "Research latest workforce planning methods", practicing: "Build talent strategies", implementing: "Present long-term strategy roadmap" },
      { month: 10, learning: "Participate in industry HRBP forums", practicing: "Lead leadership training sessions", implementing: "Measure leadership development gains" },
  ...
  // CHRO / VP of HR / Director of HR (12-18+ years)
  if (role.includes("chro") || role.includes("vp hr") || role.includes("director of hr")) {
    return [
      ...
      { month: 3, learning: "Study global compliance & governance", practicing: "Oversee global HR policy integrations", implementing: "Report status to board" },
      ...
      { month: 5, learning: "Research emerging digital transformation in HR", practicing: "Oversee digital transformation pilot project", implementing: "Track digital adoption" },
      ...
      { month: 10, learning: "Join C-suite executive committee", practicing: "Drive company-wide stakeholder engagement", implementing: "Document leadership influence" },
      { month: 11, learning: "Lead crisis management for HR response", practicing: "Oversight of legal and compliance risks", implementing: "Develop risk mitigation plans" },
  ...
  // Default plan for other roles
  return [
    { month: 1, learning: "Assess current HR skills and gaps", practicing: "Shadow senior team members", implementing: "Create personal development plan" },
    { month: 2, learning: "Study industry best practices", practicing: "Implement small process improvements", implementing: "Track and measure results" },
    { month: 3, learning: "Attend relevant training or certification", practicing: "Apply new knowledge to daily tasks", implementing: "Share learnings with team" },
    { month: 4, learning: "Build cross-functional relationships", practicing: "Collaborate on cross-team projects", implementing: "Document collaboration outcomes" },
    { month: 5, learning: "Develop expertise in specialized area", practicing: "Lead small initiatives", implementing: "Present results to management" },
    { month: 6, learning: "Seek mentorship and coaching", practicing: "Mentor junior team members", implementing: "Gather feedback on leadership style" },
    { month: 7, learning: "Study advanced HR technologies", practicing: "Pilot new tools or processes", implementing: "Evaluate and recommend improvements" },
    { month: 8, learning: "Join professional HR communities", practicing: "Network and share experiences", implementing: "Bring external insights to team" },
    { month: 9, learning: "Focus on strategic thinking skills", practicing: "Contribute to strategic planning", implementing: "Present strategic recommendations" },
    { month: 10, learning: "Develop change management capabilities", practicing: "Lead change initiatives", implementing: "Measure change adoption success" },
    { month: 11, learning: "Enhance communication and influence", practicing: "Present to senior leadership", implementing: "Gather feedback and improve" },
    { month: 12, learning: "Plan next career progression", practicing: "Set goals for following year", implementing: "Create roadmap for advancement" }
  ];
};
...
export function buildRoadmap(input: RoadmapInput): Roadmap {
  ...
  if (y <= 2) {
    nextSteps.push({
      ...
      title: "Master current role fundamentals",
      reason: "Build credibility in your current position before seeking growth",
      skillsImpacted: gaps.length > 0 ? gaps.slice(0, 2) : ["documentation", "communication"],
    })
    nextSteps.push({
      timeframe: "6-12 months",
      title: nextLikelyRole ? `Prepare for ${nextLikelyRole}` : "Seek stretch assignments",
      reason: nextLikelyRole ? "Clear progression path identified" : "Expand experience within current organization",
      skillsImpacted: nextLikelyRole ? ["leadership basics", "process improvement"] : ["project management", "cross-functional"],
    })
    nextSteps.push({
      timeframe: "12-18 months",
      title: nextLikelyRole ? `Transition to ${nextLikelyRole}` : "Apply for promotion or new role",
      reason: "Ready for increased responsibility with solid foundation",
      skillsImpacted: ["strategic thinking", "stakeholder management"],
    })
  } else if (y <= 7) {
    nextSteps.push({
      timeframe: "0-6 months",
      title: "Develop specialization",
      reason: `With ${y} years experience, focus on becoming the go-to expert in key areas`,
      skillsImpacted: gaps.length > 0 ? gaps.slice(0, 3) : ["expertise depth", "thought leadership"],
    })
    nextSteps.push({
      timeframe: "6-18 months",
      title: nextLikelyRole ? `Target ${nextLikelyRole}` : "Lead major initiatives",
      reason: nextLikelyRole ? "Natural career progression based on your profile" : "Demonstrate senior-level capabilities",
      skillsImpacted: ["team leadership", "strategic projects", "change management"],
    })
    nextSteps.push({
      timeframe: "18-36 months",
      title: "Build executive presence",
      reason: "Prepare for senior leadership roles in HR",
      skillsImpacted: ["executive communication", "business acumen", "organizational influence"],
    })
  } else {
    nextSteps.push({
      timeframe: "0-6 months",
      title: "Assess strategic positioning",
      reason: `As an experienced professional (${y} years), evaluate your career trajectory and market position`,
      skillsImpacted: ["strategic planning", "market awareness"],
    })
    nextSteps.push({
      timeframe: "6-18 months",
      title: nextLikelyRole ? `Advance to ${nextLikelyRole}` : "Pursue executive opportunities",
      reason: nextLikelyRole ? "Clear advancement path based on experience" : "Leverage extensive experience for senior roles",
      skillsImpacted: ["executive leadership", "organizational transformation", "business strategy"],
    })
    nextSteps.push({
      timeframe: "18-36 months",
      title: "Establish thought leadership",
      reason: "Shape the future of HR in your industry",
      skillsImpacted: ["industry influence", "innovation leadership", "mentoring others"],
    })
  }

  const monthlyPlan = getDetailedMonthlyPlan(nextLikelyRole || input.currentRole);
